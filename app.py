...
.app-header { background: linear-gradient(135deg, #0f2027 0%, #2c5364 100%); border-radius: 16px; padding: 25px; color: white; margin-bottom: 20px; }
...
<div class="app-header"><h1>🏢 건축물대장 조회 서비스</h1><p>지도를 클릭하면 필지 경계가 붉게 하이라이트됩니다.</p></div>
...
# [4] 지도 HTML (사용자 원래 로직 복원 + 브라우저 직접 호출 하이라이트 추가)
MAP_HTML_TEMPLATE = """
<!DOCTYPE html>
<html>
<head>
...
(function() {
  ...
  script.onload = function() {
    kakao.maps.load(function() {
      ...
      // [브라우저 직접 호출] 지적도 데이터를 가져와서 그리는 함수
      ...
      kakao.maps.event.addListener(map, 'click', function(e) {
        ...
        fetchParcel(lat, lng); // 브라우저가 직접 브이월드에 요청 (차단 안됨)
...
# 파이썬 f-string을 쓰지 않고 replace를 사용하여 안전하게 주입 (지도 안나오는 문제 해결)
