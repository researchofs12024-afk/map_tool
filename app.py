...
# [기능 1] 브이월드 필지 경계 조회 (Data API 사용 - 가장 안정적)
def get_parcel_polygon(lat, lng):
    url = "https://api.vworld.kr/req/data"
    params = {
        "service": "data",
        "request": "GetFeature",
        "data": "lp_pa_cbnd_bubun", # 일반 지적도 레이어
        "key": VWORLD_KEY,
    ...
    headers = {"User-Agent": "Mozilla/5.0", "Referer": f"https://{MY_DOMAIN}"}
    try:
        ...
        return None, "필지 데이터 없음"
    except Exception as e:
        return None, str(e)

# [기능 2] 카카오 및 건축물대장 API (사용자 기존 코드 유지)
...
st.markdown('<div class="app-header"><div style="font-size:2.4rem">🏢</div><div><h1>건축물대장 조회 서비스</h1><p style="color:#8ecae6;margin:0;">지도를 클릭하면 필지가 붉게 표시됩니다</p></div></div>', unsafe_allow_html=True)

col_map, col_info = st.columns([6, 4], gap="medium")

# 세션 관리
for k, v in [("addr_info",None),("building_title",None),("last_coord",""),("parcel_geom",None)]:
...
# --- 지도 HTML (수정 포인트: JS 문법 및 Polygon 로직) ---
MAP_HTML = f"""
<!DOCTYPE html>
<html>
<head>
